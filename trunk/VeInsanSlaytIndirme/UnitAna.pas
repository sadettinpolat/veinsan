...
begin
  ...
  ini.UpdateFile;
  FreeAndNil(ini);
  for i:=0 to cntMaxProgramSayisi do VeInsan[i].Free;
end;
...
begin
  ...
  IslemDevamEdiyor := False;
  PointerVeInsan := 0;
  for i:=0 to cntMaxProgramSayisi do VeInsan[i] := TVeInsan.Create;
end;
...
begin
  ...
  if ResimSayisi = 0 Then ShowMessage('Resimler alýnamadý...');
  ...
  Begin
    ...
    Begin
      ...
      tmp := tmp + Memo1.Lines[i+a];
      Break;
    end; //    if Pos('var Headline',Memo1.Lines[i]) >0 Then
  end;   //for i:=Konum to Memo1.Lines.Count -1 do
  ...
  begin
    ...
    Begin
      ...
      tmp := tmp + Memo1.Lines[i+a];
      Break;
    end; //    if Pos('var PhotoCaption',Memo1.Lines[i]) >0 Then
  end;
  ...
  Begin
    ...
    begin
      ...
      begin
        ...
        Memo2.Lines.Clear;
        Memo2.Lines.Add(''); //ve insan slayt goruntuleme icin bu bos satirlar gerekli....
        Memo2.Lines.Add('');
        Memo2.Lines.Add('');
...
begin
  ...
  Begin
    ...
    Begin
      ...
      if clDownLoader1.IsBusy Then clDownLoader1.Stop;
      while clDownLoader1.IsBusy do Application.ProcessMessages;
      Halt;
...
begin
  ...
  begin
    ...
    if Form2.ShowModal = mrOk Then
      if FileExists(VISGA) then ShellExecute(handle,'open',pchar(visga),nil,nil,SW_SHOWNORMAL)
  end;
end;
end.
